# Seedance 2.1 Pro — Agent Instructions
...
| `deploy-cloudflare` | Deploy this project's reviewed Supabase PostgreSQL + Hyperdrive Worker route |

**Cloudflare production route:** this project deploys only to **Supabase PostgreSQL via Hyperdrive**. The reviewed local `wrangler.jsonc` must set `vars.DATABASE_PROVIDER` to `postgresql`, bind `HYPERDRIVE`, and include the dedicated AI-edit durable-drain cron. `src/core/db/postgres.ts` reads the runtime connection from the Worker binding stashed on `globalThis.__CF_ENV__`; Vite keeps the PostgreSQL driver when building from that configuration. The tracked `wrangler.example.jsonc` is bundle-only and intentionally has a placeholder binding and no cron. `pnpm cf:deploy` refuses the template, requires actual required secrets, runs the deploy build gate, then invokes Wrangler explicitly with `--config wrangler.jsonc`. Any production database change is a separately reviewed Supabase operation, never part of the Worker deploy command.
...
VITE_APP_URL=https://seedance21pro.com
VITE_APP_NAME=Seedance 2.1 Pro
